...
from tests.utils import ALL_DESTINATIONS, assert_load_info, assert_query_data, load_table_counts, load_table_distinct_counts


@pytest.mark.parametrize('destination_name', ALL_DESTINATIONS)
def test_github_reactions(destination_name: str) -> None:
    pipeline = dlt.pipeline("github_reactions", destination=destination_name, dataset_name="duckdb_issues", full_refresh=True)
...
def test_github_events(destination_name: str) -> None:
    pipeline = dlt.pipeline("github_events", destination=destination_name, dataset_name="airflow_events", full_refresh=True)
