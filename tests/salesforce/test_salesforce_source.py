...
from sources.salesforce.helpers import _process_record


@pytest.mark.parametrize("destination_name", ALL_DESTINATIONS)
def test_all_resources(destination_name: str) -> None:
    pytest.skip(
        "If we get actual non-trial credentials, fix the expectations and unskip."
    )
    expected_tables = [
        "account",
        "campaign",
...
def test_process_record(is_bulk_api: bool) -> None:
    ...
    sample_data = OrderedDict(
        [
            ...
            (
                "CreatedDate",
                "2025-08-06T14:16:08.000+0000" if not is_bulk_api else 1754489768000,
            ),
            (
                "LastModifiedDate",
                "2025-08-06T14:16:08.000+0000" if not is_bulk_api else 1754489768000,
    ...
    processed_record = _process_record(
        sample_data, date_fields, is_bulk_api=is_bulk_api
    )
