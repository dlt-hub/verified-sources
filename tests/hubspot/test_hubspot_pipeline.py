...
from tests.utils import ALL_DESTINATIONS, assert_load_info, load_table_counts, load_table_distinct_counts
...
def test_all_resources(destination_name: str) -> None:
    ...
    assert load_table_counts(pipeline, *table_names) == load_table_distinct_counts(pipeline, "hs_object_id", *table_names) == {'companies': 200, 'deals': 500, 'contacts': 402}
...
def test_event_resources(destination_name: str) -> None:
    pipeline = dlt.pipeline(pipeline_name='hubspot', destination=destination_name, dataset_name='hubspot_data', full_refresh=True)
    load_info = pipeline.run(hubspot_events_for_objects("company", ["7086461639", "7086464459"]))
    print(load_info)
    assert_load_info(load_info)
