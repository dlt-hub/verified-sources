...
@pytest.mark.parametrize("destination_name", ALL_DESTINATIONS)
def test_load_sql_table_loads_data(sql_source_db: SQLAlchemySourceDB, destination_name: str) -> None:
    @dlt.source
    ...
    with pipeline.sql_client() as c:
        info = sql_source_db.table_infos['chat_message']
        with c.execute_query("SELECT count(*) FROM chat_message") as cur:
            row = cur.fetchone()
            assert row[0] == info['row_count']
