...
from pipelines.facebook_ads import facebook_ads_source, facebook_insights_source, enrich_ad_objects, Campaign, DEFAULT_CAMPAIGN_FIELDS
...
def test_load_all_ads_object(destination_name: str) -> None:
    pipeline = dlt.pipeline(pipeline_name='facebook_ads', destination=destination_name, dataset_name='facebook_ads_data', full_refresh=True)
    ...
    assert set(expected_tables) - set(t["name"] for t in pipeline.default_schema.data_tables()) == set()
    # get counts
