...
from facebook_ads import facebook_ads_source, facebook_insights_source, DEFAULT_ADCREATIVE_FIELDS, AdCreative, enrich_ad_objects


def load_all_ads_objects() -> None:
    """Loads campaigns, ad sets, ads, ad creatives and leads"""
    pipeline = dlt.pipeline(pipeline_name='facebook_ads', destination='duckdb', dataset_name='facebook_ads_data', full_refresh=True)
...
def load_only_disapproved_ads() -> None:
    ...
    fb_ads.ads.bind(states=("DISAPPROVED", ))  # states=("DISAPPROVED", "PAUSED") for many states
    info = pipeline.run(fb_ads.with_resources("ads"))
    print(info)
