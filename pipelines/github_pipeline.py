...
def load_duckdb_repo_reactions_issues_only() -> None:
    ...
    pipeline = dlt.pipeline("github_reactions", destination="duckdb", dataset_name="duckdb_issues", full_refresh=True)
...
def load_dlthub_dlt_all_data() -> None:
    ...
    pipeline = dlt.pipeline("github_reactions", destination="duckdb", dataset_name="dlthub_reactions", full_refresh=True)
